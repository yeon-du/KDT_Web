    ...
    >
      ...
      {targetCurrency !== "USD" && (
        <div className="border-r border-line pr-6">
          ...
          {/* This rate is cross-derived from the same Frankfurter/ECB fetch
              as USD/KRW above (usdKrw ÷ that currency's USD cross rate) —
              "선택한 통화" was describing *which field this is* rather than
              *where the number came from*, breaking the pattern every other
              chip here follows (a data source, not a field name). */}
          <small className="ml-1.5 text-[10px] text-muted">Frankfurter/ECB{sourceSuffix}</small>
        </div>
      )}
      <div className="border-r border-line pr-6">
